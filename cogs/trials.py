...
class TrialApplicationModal(discord.ui.Modal, title="Trial Application"):
    ...
    rank_experience = discord.ui.TextInput(
        label="Rank and Experience",
        style=discord.TextStyle.paragraph,
        placeholder="Current rank and a short summary of your gaming experience",
        required=True,
        max_length=500
    )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        existing = await asyncio.to_thread(db.get_pending_trials, interaction.guild.id, interaction.user.id)
        if existing:
        ...
        application_data = {
            ...
            "rank_experience": self.rank_experience.value,
            "why_join": self.why_join.value,
            "availability": self.availability.value
        }
        import json
        trial_id = await asyncio.to_thread(
            db.add_trial,
        ...
        if gatekeeper_channel_id:
            ...
            if channel:
                embed = create_trial_embed(
                    ...
                    f"**Age/Region:** {self.age.value} / {self.region.value}\n"
                    f"**Rank and Experience:** {self.rank_experience.value}\n"
                    f"**Why Join:** {self.why_join.value}\n"
                ...
                await asyncio.to_thread(db.update_trial_message_id, interaction.guild.id, trial_id, message.id)
...
class TrialAssignmentButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        trial = await asyncio.to_thread(db.get_trial, interaction.guild.id, self.trial_id)
        if not trial:
...
class TrialRejectButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        await asyncio.to_thread(db.deny_trial, interaction.guild.id, self.trial_id, interaction.user.id)
        await asyncio.to_thread(db.log_action, interaction.guild.id, "trial_rejected", interaction.user.id, self.applicant_id, "Application rejected")
...
class TrialHoldButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        await asyncio.to_thread(db.update_trial_status, interaction.guild.id, self.trial_id, "on_hold")
        await interaction.message.edit(content=f"Put on hold by {interaction.user.mention}", view=None)
        await interaction.followup.send("Application put on hold.", ephemeral=True)
...
class TrialResultModal(discord.ui.Modal, title="Trial Result"):
    ...
    rank_role = discord.ui.TextInput(
        label="Rank and Recommended Role",
        placeholder="e.g., Legendary I | Elder",
        required=True,
        max_length=100
    )
    strengths_weaknesses = discord.ui.TextInput(
        ...
        style=discord.TextStyle.paragraph,
        placeholder="Summarize the applicant's strengths and weaknesses",
        required=True,
        max_length=500
    )
...
class TrialReassignButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        # Reset assignment
        await asyncio.to_thread(db.reset_trial_assignment, interaction.guild.id, self.trial_id)
        await asyncio.to_thread(db.log_action, interaction.guild.id, "trial_reassigned", interaction.user.id, self.applicant_id, "Trial reassigned")
        
        await interaction.message.edit(content=f"Reassigned by {interaction.user.mention}", view=None)
        await interaction.followup.send("Trial reassigned to queue.", ephemeral=True)
...
class TrialCancelButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        # Cancel trial
        await asyncio.to_thread(db.deny_trial, interaction.guild.id, self.trial_id, interaction.user.id)
        await asyncio.to_thread(db.log_action, interaction.guild.id, "trial_cancelled", interaction.user.id, self.applicant_id, "Trial cancelled")
        ...
        await interaction.followup.send("Trial cancelled.", ephemeral=True)
...
class TrialButtons(discord.ui.View):
    ...
    async def approve_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        trial = await asyncio.to_thread(db.get_trial, self.guild_id, self.trial_id)
        if not trial:
        ...
        # Approve
        await asyncio.to_thread(db.approve_trial, self.guild_id, self.trial_id, interaction.user.id)
        await asyncio.to_thread(db.log_action, self.guild_id, "trial_approved", interaction.user.id, self.user_id, f"Trial approved by {interaction.user.display_name}")
    ...
    async def deny_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await asyncio.to_thread(db.deny_trial, self.guild_id, self.trial_id, interaction.user.id)
        await asyncio.to_thread(db.log_action, self.guild_id, "trial_denied", interaction.user.id, self.user_id, f"Trial denied by {interaction.user.display_name}")
...
class Trials(commands.Cog):
    ...
    async def assign_gatekeeper(self, interaction: discord.Interaction, applicant: discord.Member, gatekeeper: discord.Member):
        ...
        trials = await asyncio.to_thread(db.get_pending_trials, interaction.guild.id, applicant.id)
        if not trials:
            embed = create_error_embed(
                "No Pending Trial",
        ...
        try:
            ...
            age_region = application_data.get('age_region') or (
                f"{application_data.get('age')} / {application_data.get('region')}" if application_data.get('age') or application_data.get('region') else "Unknown"
            )
            rank_experience = application_data.get('rank_experience') or (
                f"{application_data.get('rank')} - {application_data.get('experience')}" if application_data.get('rank') or application_data.get('experience') else "Unknown"
            )
            description = (
                f"**Age/Region:** {age_region}\n"
                f"**Rank and Experience:** {rank_experience}\n"
        ...
        await asyncio.to_thread(db.assign_gatekeeper, interaction.guild.id, trial_id, gatekeeper.id)
    ...
    async def status(self, interaction: discord.Interaction):
        ...
        trials = await asyncio.to_thread(db.get_user_trials, interaction.guild.id, interaction.user.id)
    ...
    async def deletetrial(self, interaction: discord.Interaction, user: discord.Member):
        ...
        if not has_perm:
            embed = create_error_embed(
                "Permission Denied",
                "You don't have permission to delete trials."
            )
            return await interaction.response.send_message(embed=embed, ephemeral=True)
        ...
        channel_message = (
            ...
            "No associated trial channel was found or it could not be deleted."
        )
    ...
    async def trial_queue(self, interaction: discord.Interaction):
        ...
        trials = await asyncio.to_thread(db.get_pending_trials, interaction.guild.id)
...
class TrialQueueResetView(discord.ui.View):
    ...
    async def clear_inactive(self, interaction: discord.Interaction):
        ...
        old_trials = await asyncio.to_thread(
            db._fetchall,
        ...
        if not old_trials:
            return await interaction.followup.send("No inactive trials to clear.", ephemeral=True)
        
        # Clear them
        for trial in old_trials:
            await asyncio.to_thread(db.deny_trial, interaction.guild.id, trial[0], interaction.user.id)
            await asyncio.to_thread(db.log_action, interaction.guild.id, "trial_auto_denied", interaction.user.id, trial[1], "Auto-denied for inactivity")
        ...
        await interaction.followup.send(f"Cleared {len(old_trials)} inactive trials.", ephemeral=True)
...
class ResetConfirmationModal(discord.ui.Modal, title="Confirm Reset"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        pending_trials = await asyncio.to_thread(db.get_pending_trials, self.guild_id)
        for trial in pending_trials:
            await asyncio.to_thread(db.update_trial_status, self.guild_id, trial[0], "pending")  # Reset to pending
            await asyncio.to_thread(db.log_action, self.guild_id, "trial_reset", interaction.user.id, trial[2], "Trial queue reset")
        
        await interaction.followup.send(f"Reset {len(pending_trials)} trials to pending status.", ephemeral=True)
